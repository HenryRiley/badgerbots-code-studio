...
Host 0.7.0 manages operational snapshots for four fixed world roots: the teacher overworld,
...
A backup becomes visible only after all staged files match its manifest. Host retains the newest
five operational snapshots and automatically creates one before a normal server start. Restore
re-verifies the latest snapshot, stages a complete replacement, moves current world roots into a
rollback directory, and then commits only fixed world names. A failure attempts to restore every
previous root.
